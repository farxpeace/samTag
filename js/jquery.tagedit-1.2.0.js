						...
						$(this).bind('transformToTag', function(event, id) {
							...
							if(isNewResult[0] === true || (isNewResult[0] === false && typeof isNewResult[1] == 'string')) {
								...
								if(options.allowAdd == true || oldValue) {
									...
									html += '<input name="search['+id+']" id="'+uniqueID+'" ref="'+id+'" type="text" style="margin-left: 10px;" />';
									...
									//autocomplete
									var phpFunction = input.attr('ref');
									input.autocomplete({
										source: "index.php?m=bulk&c=shw_ajax_request&phpFunction="+phpFunction+"&extraData=ajax",
										minLength: 2,
										select: function( event, ui ) {
											var newInput = $('.tagedit-listelement-new').children('#tagedit-input');
											newInput.focus().val(newInput.val());
										}
									});
